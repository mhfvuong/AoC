        ...
        for j in 0..grid[0].len() {
            ...
            for x in j+1..grid.len(){
                if grid[i][x] >= grid[i][j] {
                    // println!("x0: {} is taller than {}", &grid[i][x], &grid[i][j]);
                    taller_x[0] = true;
                    if !taller_x[0] {
                        score_x[0] += 1;
                    }
            ...
            while x > 0 {
                x -= 1;
                if grid[i][x] >= grid[i][j] {
                    // println!("x1: {} is taller than {}", &grid[i][x], &grid[i][j]);
                    taller_x[1] = true;
                    if !taller_x[1] {
                        score_x[1] += 1;
                    }
            ...
            for y in i+1..grid.len(){
                if grid[y][j] >= grid[i][j] {
                    // println!("y0: {} is taller than {}", &grid[y][j], &grid[i][j]);
                    taller_y[0] = true;
                    if !taller_y[0] {
                        score_y[0] += 1;
                    }
            ...
            while y > 0 {
                y -= 1;
                if grid[y][j] >= grid[i][j] {
                    // println!("y1: {} is taller than {}", &grid[y][j], &grid[i][j]);
                    taller_y[1] = true;
                    if !taller_y[1] {
                        score_y[1] += 1;
                    }
